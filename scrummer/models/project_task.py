...
class ProjectTaskLink(models.Model):
    ...
    relation_id = fields.Many2one(
        comodel_name="project.task.link.relation", scrummer=True
    )

    task_left_id = fields.Many2one(comodel_name="project.task", scrummer=True,)

    task_right_id = fields.Many2one(
        comodel_name="project.task", scrummer=True,
    )
...
class TaskType(models.Model):
    ...
    priority_ids = fields.Many2many(
        comodel_name="project.task.priority", scrummer=True
    )
...
class Task(models.Model):
    ...
    project_id = fields.Many2one(comodel_name="project.project", scrummer=True)
    parent_id = fields.Many2one(comodel_name="project.task", scrummer=True)
    stage_id = fields.Many2one(comodel_name="project.task.type", scrummer=True)
    ...
    child_ids = fields.One2many(comodel_name="project.task", scrummer=True)
    timesheet_ids = fields.One2many(
        comodel_name="account.analytic.line", scrummer=True
    )
    attachment_ids = fields.One2many(
        comodel_name="ir.attachment", inverse_name="res_id", scrummer=True
    )
    tag_ids = fields.Many2many(comodel_name="project.tags", scrummer=True)
    type_id = fields.Many2one(
        comodel_name="project.task.type2", scrummer=True,
    )
    resolution_id = fields.Many2one(scrummer=True,)
    project_type_id = fields.Many2one(
        comodel_name="project.type", scrummer=True,
    )
    epic_id = fields.Many2one(comodel_name="project.task", scrummer=True,)
    user_id = fields.Many2one(scrummer=True)
    create_uid = fields.Many2one(comodel_name="res.users", scrummer=True,)
    ...
    project_last_update = fields.Datetime(
        related="project_id.__last_update", readonly=True, scrummer=True
    )
    user_last_update = fields.Datetime(
        related="user_id.__last_update", readonly=True, scrummer=True
    )
    stage_name = fields.Char(
        related="stage_id.name", readonly=True, scrummer=True
    )
    parent_key = fields.Char(
        related="parent_id.key", readonly=True, scrummer=True
    )
